...
"""三、將Inception-V3模型,修改輸出層為7個類別並設置learning rate scheduler和Checkpoint做特徵提取"""

# # 修改 Inception-V3 模型以適應 7 個輸出類別
# # 使用最新的權重參數設置方法
...
# # # 訓練模型
# num_epochs = 25
# best_model, history = train_model(model, dataloaders, criterion, optimizer, exp_lr_scheduler, num_epochs, log_file="inceptionV3_fe.txt")

# # 保存最佳模型權重
# torch.save(best_model.state_dict(), 'inceptionv3_feature_extraction.pth')
...
# plt.savefig('inceptionv3_feature_extraction.png')
# plt.clf()
...
# 確保模型權重載入時符合安全性建議
model.load_state_dict(torch.load('inceptionv3_feature_extraction.pth'))
...
finetuned_model, history_fine_tune = train_model(
    model, dataloaders, criterion, optimizer, scheduler, num_epochs=num_epochs_fine_tune, log_file="inceptionV3_ft.txt"
)

# 保存微調後的模型權重
torch.save(finetuned_model.state_dict(), 'inceptionv3_finetuned.pth')
...
plt.savefig('inceptionv3_finetuning.png')
plt.clf()
