...
# 解凍 EfficientNet-B2 最後幾個卷積層進行微調
    ...
    for inputs, labels in dataloaders['test']:
        ...
        for i in range(inputs.size(0)):
            ...
            if class_display_count[class_name] < images_per_class:
                ...
                # 將 Tensor 移至 CPU 並轉換為 numpy 格式
                img = inputs[i].cpu().permute(1, 2, 0).numpy()
                img = img * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]  # 反正規化
                img = np.clip(img, 0, 1)
                ...
                try:
                    ...
                    print(f"row: {row}, col: {col}, class_name: {class_name}")
