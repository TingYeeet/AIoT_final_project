...
val_test_transform = transforms.Compose([
    transforms.Resize((227, 227)),  # 調整到固定大小
    transforms.ToTensor(),
...
# # 只優化未凍結的層（即全連接層）
# optimizer = optim.SGD(model.classifier[1].parameters(), lr=0.00005, momentum=0.9, weight_decay=0.2)
...
# # 訓練模型
# num_epochs = 25
# best_model, history = train_model(model, dataloaders, criterion, optimizer, exp_lr_scheduler, num_epochs, log_file="squeezenet_fe.txt")

# # 保存最佳模型權重
# torch.save(best_model.state_dict(), 'squeezenet_feature_extraction.pth')
...
# plt.savefig('squeezenet_feature_extraction.png')
# plt.clf()
...
# 載入之前保存的權重
model.load_state_dict(torch.load('squeezenet_feature_extraction.pth'))

# 鎖定所有層的參數
for param in model.parameters():
    param.requires_grad = False

# 解凍 SqueezeNet 的最後兩個卷積層
for name, param in model.named_parameters():
    if "features.12" in name:  # SqueezeNet 的最後兩個卷積層是 features.11 和 features.12
...
# 定義優化器和學習率調度器
optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=0.00002, momentum=0.9, weight_decay=0.4)
...
finetuned_model, history_fine_tune = train_model(
    model, dataloaders, criterion, optimizer, scheduler, num_epochs=num_epochs_fine_tune, log_file="squeezenet_ft.txt"
)

# 保存微調後的模型權重
torch.save(finetuned_model.state_dict(), 'squeezenet_finetuned.pth')
...
plt.savefig('squeezenet_finetuning.png')
plt.clf()
