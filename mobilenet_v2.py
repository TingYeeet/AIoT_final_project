...
"""四、將SqueezeNet模型,修改輸出層為7個類別並設置learning rate scheduler和Checkpoint做特徵提取"""

# # 載入 MobileNetV2 預訓練權重
# weights = models.MobileNet_V2_Weights.IMAGENET1K_V1
# model = models.mobilenet_v2(weights=weights)

# # 修改分類器以適應新的類別數量
# # MobileNetV2 的分類器默認是一個 `nn.Sequential` 結構，最後一層是全連接層
# num_ftrs = model.last_channel  # MobileNetV2 的輸出通道數 (通常是 1280)
# model.classifier[1] = nn.Linear(num_ftrs, 7)  # 替換最後的全連接層為新的類別數

# # 凍結所有卷積層的參數
# for name, param in model.named_parameters():
#     if "classifier" not in name:  # 除了分類器層，其餘參數都設置為不可訓練
...
# # 只優化未凍結的層（即全連接層）
# optimizer = optim.SGD(model.classifier[1].parameters(), lr=0.0001, momentum=0.9, weight_decay=0.05)
...
# # 訓練模型
# num_epochs = 25
# best_model, history = train_model(model, dataloaders, criterion, optimizer, exp_lr_scheduler, num_epochs, log_file="mobilenetv2_fe.txt")

# # 保存最佳模型權重
# torch.save(best_model.state_dict(), 'mobilenetv2_feature_extraction.pth')
...
# plt.savefig('mobilenetv2_feature_extraction.png')
# plt.clf()

"""五、進行fine-tuning"""

# 解凍 SqueezeNet 最後幾個卷積層進行微調
# 假設特徵提取的最佳模型已經訓練完成並保存為 "resnet18_feature_extraction.pth"

# 初始化 MobileNetV2 模型（不載入預訓練權重）
...
# 載入之前保存的權重
model.load_state_dict(torch.load('mobilenetv2_feature_extraction.pth'))
...
# 定義優化器和學習率調度器
optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=0.00008, momentum=0.9, weight_decay=0.001)
...
finetuned_model, history_fine_tune = train_model(
    model, dataloaders, criterion, optimizer, scheduler, num_epochs=num_epochs_fine_tune, log_file="mobilenetv2_ft.txt"
)

# 保存微調後的模型權重
torch.save(finetuned_model.state_dict(), 'mobilenetv2_finetuned.pth')
...
plt.savefig('mobilenetv2_finetuning.png')
plt.clf()
